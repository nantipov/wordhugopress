...
RUN cd /tmp && git clone https://github.com/docker-library/mysql.git
RUN cd /tmp/mysql/8.0 && cp -R config/ /etc/mysql/
RUN cd /tmp/mysql/8.0 && cp docker-entrypoint.sh /usr/local/bin/
...
#FROM registry.opensource.zalan.do/stups/openjdk:1.8.0-212-21
#LABEL maintainer="Team Broadway <broadway@zalando.de>"
#
#EXPOSE 7979
#EXPOSE 8080
#
#ENV HOME=/tmp
#
## Copy assembled content into container
#COPY api/blackhawk-ums-adapter-api.yaml /zalando-apis/blackhawk-ums-adapter-api.yaml
#COPY build/libs/blackhawk-ums-adapter-CONTINUOUS-SNAPSHOT.jar /service.jar
#
#USER 999
#CMD java -server $(/java-utils/java-dynamic-memory-opts) -jar /service.jar
